"""Script for several visualizations with PHATE (APP group).
...
from utils import load_data_phate, variable_panel, meld_panel, RANDOM_STATE, view_trajectories_group, \
...
# Number of clusters (4 for MEX only, 6 for ALL)
N_CLUSTERS = 4
N_TS_CLUSTERS = 4

# PHATE parameters (all: 20/40/15/0), (mex: 30/60/10/0), (non-mex: 20/60/30/0)
KNN = 30  # Increase connectivity
T = 60  # Increase to tighten the branches
ALPHA = 10  # Decrease to tighten and decrease distances between clusters (inverse to KNN)
GAMMA = 0  # Default = 1
...
cluster_box_plots(y[continuous], kw_test=True, n_cols=4)
...
for index in table.index:
    ...
    table.loc[index, "Total_Time_PP_Median"] = cluster['Total_Time_PP'].median(skipna=True)
    table.loc[index, "Total_Time_PP_IQR"] = cluster['Total_Time_PP'].quantile(0.75) - cluster['Total_Time_PP'].quantile(0.25)
    table.loc[index, "Avg_Daily_Time_PP_Mean"] = cluster["Avg_Daily_Time_PP"].mean(skipna=True)
    table.loc[index, "Avg_Daily_Time_PP_SD"] = cluster["Avg_Daily_Time_PP"].std(skipna=True)
    table.loc[index, "Avg_Daily_Time_PP_Adjusted_Mean"] = cluster["Avg_Daily_Time_PP_Adjusted"].mean(skipna=True)
    table.loc[index, "Avg_Daily_Time_PP_Adjusted_SD"] = cluster["Avg_Daily_Time_PP_Adjusted"].std(skipna=True)
    ...
    table.loc[index, 'X48_ROX_change_Mean'] = cluster["X48e_Delta_Post_PP_ROX"].mean(skipna=True)
    table.loc[index, 'X48_ROX_change_SD'] = cluster["X48e_Delta_Post_PP_ROX"].std(skipna=True)
...
with pd.ExcelWriter(DATA_FILE, mode='a', if_sheet_exists="replace") as writer:
    table.set_index('CLUSTER_' + table.index.astype(str)).transpose().to_excel(writer, float_format="%.2f",
                                                                               sheet_name='TS_Clusters_Table_APP')
